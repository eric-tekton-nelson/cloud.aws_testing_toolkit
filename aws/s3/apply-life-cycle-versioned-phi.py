...
# Define the lifecycle policy with Expiration days set to 3650 (10 years)
lifecycle_policy = {
    'Rules': [
        {
            'ID': 'Life Cycle for S3 versioned data older than 90 days and then transitioned to Glacier for 10 yrs',
            'Status': 'Enabled',
            'Filter': {
                'Prefix': ""  # Applies the rule to all objects in the bucket
            },
            'NoncurrentVersionExpiration': {
                'NoncurrentDays': 365,  # Expire non-current versions after 365 days
            ...
            'Expiration': {
                'Days': 3650  # Expiration now set to 3650 days (10 years) for current versions
            }
        }
    ]
}
